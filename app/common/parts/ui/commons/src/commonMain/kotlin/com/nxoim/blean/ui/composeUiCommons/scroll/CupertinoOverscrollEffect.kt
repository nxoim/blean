...
object CupertinoOverscrollEffectDefaults {
    fun overscrollSpring() = spring<Float>(stiffness = 120f)
}
/**
    ...
    private suspend fun playSpringAnimation(
        ...
        springAnimationScope?.run {
            AnimationState(
                ...
                when (reason) {
                    // boost the spring
                    CupertinoSpringAnimationReason.FLING_FROM_OVERSCROLL -> -(initialValue * 4f)
                    CupertinoSpringAnimationReason.POSSIBLE_SPRING_IN_THE_END -> initialVelocity / density
                }
            ).animateTo(
                targetValue = 0f,
                ...
                // NOTE: THE BLOCK BELOW CAUSED ISSUES WHEN INITIAL
                // VELOCITY WAS 0 AND TARGET WAS REACHED EVEN THOUGH
                // ANIMATION WAS NOT FINISHED. Uncomment if issues encountered
